...
const AllUploadedVideos = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const response = await axios.get('https://streamapibackend.onrender.com/api/videos');
  ...
  const handleDelete = async (videoId) => {
    try {
      await axios.delete(`https://streamapibackend.onrender.com/api/videos/${videoId}`);
